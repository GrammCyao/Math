...
\begin{lemma}[The measure axioms]\label{thm:the measure axioms}
    \quad
    \begin{enumerate}
        \item (Empty set) $m(\emptyset) = 0$.
...
\begin{theorem}[Monotone convergence thm for measurable sets]\label{thm:monotone convergence theorem for measurable sets}
    \quad
    \begin{enumerate}
...
\begin{comment}
For the countable subadditivity, let $E_1, E_2, \cdots$ be the sequence of Lebesgue measurable sets. (This is similar to prove as Lemma \ref{thm:the outer measure axioms}(iii).) By outer regularity, we can find a sequence $(U_n)_{n = 1}^{\infty}$ of open sets such that $E_n \subset U_n$ and $m(E_n) + \varepsilon/2^n \geq m(U_n)$. Since each open set $U_n$ can be represented as the countable union of almost disjoint boxes $B_{n, k}$. Then we have
    \begin{align*}
        m\Big(\bigcup_{n = 1}^{\infty}E_n\Big)
        &\leq m\Big(\bigcup_{n = 1}^{\infty}U_n\Big)\\
        &= m\Big(\bigcup_{n = 1}^{\infty}\bigcup_{k = 1}^{\infty}B_{n, k}\Big)\\
        &\leq \sum_{n = 1}^{\infty}\sum_{k = 1}^{\infty}|B_{n, k}|\\
        &= \sum_{n = 1}^{\infty}m(U_n)\\
        &\leq \sum_{n = 1}^{\infty}\Big(m(E_n) + \frac{\varepsilon}{2^n}\Big)\\
        &= \sum_{n = 1}^{\infty}m(E_n) + \varepsilon,
    \end{align*}
where the first line above holds for the monotonicity of Lebesgue measure and $\bigcup_{n = 1}^{\infty}E_n \subset \bigcup_{n = 1}^{\infty}U_n$; third line holds for the countable subadditivity of outer measure and Lemma \ref{thm:outer measure of elementary sets}; fourth line holds for the countable additivity of Lebesgue measure. As $\varepsilon$ was arbitrary, we have $m(\bigcup_{n = 1}^{\infty}E_n) \leq \sum_{n = 1}^{\infty}m(E_n)$.\qed
\end{comment}
